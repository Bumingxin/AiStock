...
- `scripts/enhanced_analysis_pipeline.py`：**🚨 强制使用！首选完整流水线入口**，集成AnySearch实时新闻/公告/研报/情绪搜索，串联抓数据→评分→自动同行→AnySearch增强→多智能体博弈→HTML渲染→完整性验收，避免漏步骤。**禁止绕过enhanced_analysis_pipeline直接使用generate_stock_dashboard.py**（除非enhanced_analysis_pipeline不可用，用作降级备用）。
- `scripts/generate_stock_dashboard.py`：**备用/基础流水线入口**（不带AnySearch增强），仅当enhanced_analysis_pipeline不可用时作为降级方案。
- `scripts/anysearch_enhancer.py`：AnySearch集成核心模块，为舆情/游资/风控/技术/筹码/大单分析师提供实时搜索数据。
- `scripts/enhanced_debate.py`：AnySearch增强版多智能体博弈引擎。
...
### AnySearch（全市场通用，强制使用）

- **任何分析必须使用AnySearch获取以下增量数据**（通过enhanced_analysis_pipeline.py自动集成，或手工调用anysearch_cli）：
  - 实时新闻和公司公告：搜索股票名称/代码相关的最新消息。
  - 研报和分析师观点：搜索券商研报、目标价调整。
  - 市场情绪分析：板块热度、资金关注度、社交讨论。
  - K线异常原因调查：对>10%异常波动，用AnySearch搜索同期新闻/公告。
  - 风险事件交叉验证：减持、诉讼、监管问询等。
  - 大单异动/资金流向线索：从新闻中间接判断。
- AnySearch结果必须用于增强博弈分析中的舆情分析师、游资分析师、风控分析师、大单异动监控师角色。
...
**🚨 强制要求：默认必须使用AnySearch增强版流水线**，禁止绕过anysearch直接跑基础流水线。

```bash
# 🚨 强制使用！集成AnySearch实时新闻/公告/研报/情绪搜索
python3 scripts/enhanced_analysis_pipeline.py --code 002131 --market a --industry 科技
```

可选参数：`--market auto|a|hk|us`、`--industry 科技`（自动识别可省略）、`--quick`、`--no-debate`。

增强版流水线会自动完成：行情/估值/最新财报/主营/现金流/K线 → 评分 → 自动同行 → **AnySearch实时搜索（新闻+公告+研报+情绪+K线异常原因）** → **AnySearch增强多智能体博弈** → HTML 渲染 → 完整性验收。输出 JSON 里的 `html` 和 `media_line` 是最终附件路径。

**降级条件**：仅当 `enhanced_analysis_pipeline.py` 因环境/依赖不可用时，才允许降级使用基础流水线：

```bash
# ⚠️ 降级方案：不带AnySearch增强
...
### 7. 多智能体博弈（默认开启，必须集成AnySearch）

**🚨 博弈分析必须使用AnySearch增强版**，通过AnySearch搜索实时新闻、公告、研报、情绪数据来增强六个分析师角色的判断。

**首选方案**：使用 `enhanced_analysis_pipeline.py`，它会自动运行 `enhanced_debate.py`（AnySearch增强版博弈引擎），为每个角色注入实时搜索数据。

**手工分步（若enhanced_analysis_pipeline不可用）**：先完成数据拉取、评分、同行合并，再运行AnySearch增强博弈：

```bash
# AnySearch增强博弈（推荐）
python3 scripts/enhanced_debate.py --input stock_work/XXXXXX_dash.json --out stock_work/XXXXXX_debate.json

# 若enhanced_debate不可用，再降级到标准博弈
python3 scripts/debate_engine.py --input stock_work/XXXXXX_dash.json --out stock_work/XXXXXX_debate.json
```
...
- **🚨 是否使用了 `enhanced_analysis_pipeline.py`（AnySearch增强版）？** 如果用了 `generate_stock_dashboard.py`（基础版），是否说明降级原因？
- **🚨 博弈分析是否集成了AnySearch实时搜索数据？** 舆情分析师/游资分析师/风控分析师/大单异动监控师是否有基于AnySearch搜索结果的判断？
- **🚨 K线异常是否用AnySearch搜索了同期新闻/公告来解释原因？**
