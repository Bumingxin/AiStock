...
<section class="section"><div class="section-title"><h2>🧠 一屏速读</h2><small>先看结论，再看证据</small></div><div class="summary-grid">{summary_tiles(d.get('summary'))}</div></section>
...
<section class="section grid g2"><div class="card good"><h2>🔥 催化剂</h2>{short_lines(d.get('catalysts'),5)}</div><div class="card neutral"><h2>🎯 跟踪指标</h2>{short_lines(d.get('trackers'),5)}</div></section>
