...
def _local_final_from_reports(
    ...
    viable = [
        ...
        if report.get("trend_stage") not in ("分析失败", "数据不足", "退市/停牌", "")
...
def _analyze_one_stock(code: str, name: str, sec_name: str, sec_logic: str, max_retries: int = 2) -> Dict[str, Any]:
    ...
    if "已退市或停牌" in price_text or "已跳过分析" in price_text:
        return {
            "code": code, "name": name, "sector": sec_name, "sector_logic": sec_logic,
            "trend_stage": "退市/停牌", "signal_strength": 0,
            "signal_description": "该股票已退市或停牌，已从候选池中移除。",
            "current_price": 0.0, "entry_price": 0.0, "stop_loss_price": 0.0, "target_price_3d": 0.0,
        }
    last_err = None
...
def run_pipeline(cfg: dict, stage_cb=None, cancel_event=None) -> Dict[str, Any]:
    ...
    def _check_cancel():
        if cancel_event and cancel_event.is_set():
            raise PipelineCancelled("用户取消了分析任务")
    ...
    news_result = analyze_news_to_sectors(raw, top_k=top_sectors, progress_cb=_news_progress, cancel_event=cancel_event)
    ...
    for idx, sec in enumerate(recommended[:top_sectors], start=1):
        ...
        try:
            pick = analyze_sector_to_stocks(sec_name, sec_logic, pool_names[:200], top_k=pick_count, cancel_event=cancel_event)
        except Exception as e:
    ...
    if need_fallback and all_reports:
        ...
        viable = [r for r in fallback_source
                  if r.get("trend_stage") not in ("分析失败", "数据不足", "退市/停牌", "")
