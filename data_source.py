...
def _is_stock_valid_from_tencent(raw_text: str) -> bool:
    """根据腾讯行情原始返回判断股票是否有效（非退市/停牌）。"""
    if '"' not in raw_text:
        return False
...
def _batch_validate_stocks(codes: list[str]) -> list[str]:
    """批量验证股票有效性，过滤退市/停牌股。返回有效代码列表。"""
    if not codes:
        return codes
    valid = []
    batch_size = 50
    for i in range(0, len(codes), batch_size):
        ...
        try:
            ...
            for line in text.strip().split(";"):
                ...
                name = fields[1] if len(fields) > 1 else ""
                if not code or not _is_mainboard_code(code):
                    continue
                try:
...
def get_sector_stock_pool(sector_name: str) -> list[str]:
    ...
    try:
        ...
        return _batch_validate_stocks(mainboard[:200])
    ...
    try:
        ...
        if len(fields) > 40:
            ...
            try:
                ...
                current_price = 0
            if current_price <= 0 or "退" in stock_name:
                return f"该股票({code} {stock_name})已退市或停牌，无有效行情数据，已跳过分析。"
            parts.append(
            ...
            return f"该股票({code})行情数据异常，可能已退市或停牌，已跳过分析。"
    except Exception as e:
