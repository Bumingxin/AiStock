    ...
    try:
        ...
        conn.execute("""
            CREATE TABLE IF NOT EXISTS analysis_history (
                ...
                report_type TEXT NOT NULL DEFAULT 'quantitative',
...
def _migrate_db(conn: sqlite3.Connection):
    ...
    try:
        ...
        history_cols = {r[1] for r in conn.execute("PRAGMA table_info(analysis_history)").fetchall()}
        history_columns = {
            "report_type": "TEXT NOT NULL DEFAULT 'quantitative'",
            "points_cost": "INTEGER NOT NULL DEFAULT 0",
            "stock_code": "TEXT NOT NULL DEFAULT ''",
        ...
        for name, definition in history_columns.items():
            if name not in history_cols:
                conn.execute(f"ALTER TABLE analysis_history ADD COLUMN {name} {definition}")
        conn.commit()
    except Exception:
        pass
...
def save_analysis(
    ...
    news_summary: str,
    disclaimer: str,
    report_type: str = "quantitative",
    points_cost: int = 0,
    stock_code: str = "",
    report_file: str = "",
    summary_json: str = "{}",
) -> int:
    """Save an analysis result and return its history id."""
    conn = get_conn()
    try:
        cursor = conn.execute(
            """INSERT INTO analysis_history (
                   user_id, created_at, title, final_list, all_reports, market,
                   news_summary, disclaimer, report_type, points_cost,
                   stock_code, report_file, summary_json
               ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                user_id, _now_bjt(), title, final_list, all_reports, market,
                news_summary, disclaimer, report_type, int(points_cost or 0),
                stock_code, report_file, summary_json,
            ),
        )
        conn.commit()
...
def get_analysis_history(user_id: int, limit: int = 50) -> List[Dict[str, Any]]:
    """Get a unified list of the user's quantitative and deep reports."""
    conn = get_conn()
    try:
        rows = conn.execute(
            """SELECT id, created_at, title, disclaimer, report_type,
                      points_cost, stock_code
        ...
        for row in rows:
            item = dict(row)
            item["report_type"] = item.get("report_type") or "quantitative"
            if item["report_type"] == "quantitative" and not item.get("points_cost"):
                ...
                item["points_cost"] = log_row["points_cost"] if log_row else 0
            result.append(item)
        return result
    finally:
        conn.close()
...
def get_analysis_detail(history_id: int, user_id: int) -> Optional[Dict[str, Any]]:
    """Get a report owned by the user."""
    conn = get_conn()
    try:
        row = conn.execute(
...
def get_deep_analysis_by_file(report_file: str, user_id: int) -> Optional[Dict[str, Any]]:
    """Get a deep report by stored filename and owner."""
    conn = get_conn()
    try:
        row = conn.execute(
            """SELECT * FROM analysis_history
               WHERE report_type = 'deep' AND report_file = ? AND user_id = ?""",
            (report_file, user_id),
        ).fetchone()
        return dict(row) if row else None
    finally:
