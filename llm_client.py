...
def analyze_news_to_sectors(news_text: str, top_k: int = 5, progress_cb=None, cancel_event=None) -> Any:
    client = _client()
...
def analyze_sector_to_stocks(sector_name: str, sector_logic: str, candidate_names: list[str], top_k: int = 15, cancel_event=None) -> Any:
    if cancel_event and cancel_event.is_set():
        raise RuntimeError("用户取消了分析任务")
    client = _client()
    schema = json.dumps({
